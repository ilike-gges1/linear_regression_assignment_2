...
library(ggplot2)
ggplot() +
  geom_point(aes(x = lr_data$YearsExperience, y = lr_data$Salary), colour = 'red') +
  geom_line(aes(x = lr_data$YearsExperience, y = predict(model, newdata = lr_data)), colour = 'blue') +
